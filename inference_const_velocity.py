...
in_test_loader = DataLoader(in_test_dataset, batch_size=32, shuffle=False, collate_fn=pad_collate_fn, num_workers=8, pin_memory=True)
...
out_test_loader = DataLoader(out_test_dataset, batch_size=32, shuffle=False, collate_fn=pad_collate_fn, num_workers=8, pin_memory=True)
...
for i, (data, _) in tqdm(enumerate(in_test_loader)):
    ...
    x = data[:,:8,:,:]
    y = data[:, 2:, :, (2,3,13,14)] / 255
    y_pred = (2 * data[:, 1:-1, :, (2,3,13,14)] -  data[:, :-2, :, (2,3,13,14)]) / 255    
    ...
    l2 = ((y_pred[:,:,:,[0,1]]*mask-y[:,:,:,[0,1]]*mask) ** 2).detach().sum().cpu().flatten()
    l1 = torch.abs(y_pred[:,:,:,[0,1]]*mask-y[:,:,:,[0,1]]*mask).detach().sum().cpu().flatten()
    l2_loss_track += l2
    l1_loss_track += l1

    l2 = ((y_pred[:,:,:,[2,3]]*mask-y[:,:,:,[2,3]]*mask) ** 2).detach().sum().cpu().flatten()
    l1 = torch.abs(y_pred[:,:,:,[2,3]]*mask-y[:,:,:,[2,3]]*mask).detach().sum().cpu().flatten()
...
# pickle.dump([l2_loss_track, l1_loss_track, l2_loss_intention, l1_loss_intention, count], open("result_indomain_constvelocity.pkl", 'wb'))
